...
std::optional<VersionNotSupported> ReadVersion(std::istream& ins, std::string& version)
{
    ...
    if (trimmed_line.size() < (Prefix.size() + 1)
        || std::string_view(&trimmed_line[0], Prefix.size()) != Prefix) {
        return VersionNotSupported { std::move(line) };
    }
    version = std::string(std::next(trimmed_line.begin(), static_cast<long>(Prefix.size())), trimmed_line.end());
    return std::nullopt;
}
...
public:
	...
	inline std::string&& type() { return std::move(m_FM.at(Type_str)); }
	inline std::string const& type() const { return m_FM.at(Type_str); }
...
Result ReadRecords(std::istream& in)
{
    ...
    if (record.content_length() > 0) {
        std::streamsize len = static_cast<std::streamsize>(record.content_length());
        record.m_content.resize(static_cast<unsigned long>(len));
