...
std::shared_ptr<Token> SentinelLeftParenthesis = std::make_shared<Token>(
    Token {
        .id = LeftParenthesis, .associative = RightAssociative, .Number = 0, .Precedence = 100 });

std::shared_ptr<Token> SentinelRightParenthesis = std::make_shared<Token>(
    Token {
        .id = RightParenthesis, .associative = LeftAssociative, .Number = 0, .Precedence = 100 });

std::shared_ptr<Token> SentinelExponential = std::make_shared<Token>(
    Token {
        .id = Exponential, .associative = RightAssociative, .Number = 0, .Precedence = 5, .F = ExponentionalFunc });

std::shared_ptr<Token> SentinelMultiplication = std::make_shared<Token>(
    Token {
        .id = Multiplication, .associative = LeftAssociative, .Number = 0, .Precedence = 4, .F = MultiplicationFunc });

std::shared_ptr<Token> SentinelDivison = std::make_shared<Token>(
    Token {
        .id = Division, .associative = LeftAssociative, .Number = 0, .Precedence = 4, .F = DivisonFunc });

std::shared_ptr<Token> SentinelSubstraction = std::make_shared<Token>(
    Token {
        .id = Subtraction, .associative = LeftAssociative, .Number = 0, .Precedence = 2, .F = SubtractionFunc });

std::shared_ptr<Token> SentinelAddition = std::make_shared<Token>(
    Token {
        .id = Addition, .associative = LeftAssociative, .Number = 0, .Precedence = 2, .F = AdditionFunc });
