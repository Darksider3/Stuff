...
#include <deque>
  ...
  public:
    ...
    explicit Threader(std::atomic_bool &GlobalStopVar) : globalStop(GlobalStopVar)
    {}
    ...
    std::shared_ptr<std::thread> RunThread(Threader &)
    {
      std::shared_ptr<std::thread> &&MovingThread = std::make_shared<std::thread>(&Threader::threadingFunction, std::ref(*this));
...
int main()
{
  ...
  Threader ThreaderWrapper(Global);
  std::shared_ptr<std::thread> ThreadCopy = ThreaderWrapper.RunThread(ThreaderWrapper);
  std::this_thread::sleep_for(std::chrono::milliseconds(500));
